...
class SettingNode
{
    ...
    public function addLeveldChildren(string $name, string $description, LeveledNodeConfig $level0_conf, LeveledNodeConfig $level1_conf, LeveledNodeConfig $level2_conf, LeveledNodeConfig $level3_conf, LeveledNodeConfig $levelOver3_conf): ?LeveledSettingNode
    {
        /*$this->addChild("level_0", null, null, "settings for level 0.")
            ->addArchiveChildren($level0_conf->getState(), $level0_conf->getUnit(), $level0_conf->getValue());

        $this->addChild("level_1", null, null, "settings for level 1.")
            ->addArchiveChildren($level1_conf->getState(), $level1_conf->getUnit(), $level1_conf->getValue());

        $this->addChild("level_2", null, null, "settings for level 2.")
            ->addArchiveChildren($level2_conf->getState(), $level2_conf->getUnit(), $level2_conf->getValue());

        $this->addChild("level_3", null, null, "settings for level 3.")
            ->addArchiveChildren($level3_conf->getState(), $level3_conf->getUnit(), $level3_conf->getValue());

        $this->addChild("level_over_3", null, null, "settings for level over 3.")
            ->addArchiveChildren($levelOver3_conf->getState(), $levelOver3_conf->getUnit(), $levelOver3_conf->getValue());*/

        if ( is_null($this->getChild($name)) ) {

            $node = new LeveledSettingNode($this, $name, $description, $level0_conf, $level1_conf, $level2_conf, $level3_conf, $levelOver3_conf);
    ...
    protected function addArchiveChildren(LevelState $state, LevelUnit $unit, string $value, LevelAction $action): static
    {
        ...
        $this->addChild("action", $action->value, "string", $this->getNameNormalized() . " action.");
