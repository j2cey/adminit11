...
use App\Traits\ItemDoable\HasItemsDoable;
use App\Contracts\ItemDoable\IHasItemsDoable;
 ...
 *
 * @property string $items_done
 * @property string $items_succeed
 * @property string $items_failed
 *
 ...
 */
class ImportResult extends BaseModel implements Auditable, IHasItemsDoable
{
    use HasFactory, HasItemsDoable, \OwenIt\Auditing\Auditable;
    ...
    public function addOneToImport() {
        $this->update(['nb_to_import' => $this->nb_to_import+= 1,]
        );
        $this->setImportUpToDate(true);

        $this->upperimportresult?->addOneToImport();
    }

    public function addManyToImport(int $nb_to_import) {
        $this->update(['nb_to_import' => $this->nb_to_import+= $nb_to_import,]
        );
        $this->setImportUpToDate(true);

        $this->upperimportresult?->addManyToImport($nb_to_import);
    }

    public function itemAlreadyImported(int $item): bool {
        return $this->itemIsAlreadyDone($item);
    }
    ...
    public function items_done_field(): string
    {
        return "items_done";
    }

    public function items_succeed_field(): string
    {
        return "items_succeed";
    }

    public function items_failed_field(): string
    {
        return "items_failed";
    }
}
