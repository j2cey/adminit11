...
trait TreatmentEnding
{
    ...
    public function  doEnding(TreatmentResultEnum $treatmentresult, string|null $message, Treatment|null $child_treatment, bool $child_completed = false) {
        ...
        $this->addExecTrace($this->name, $this->code->value, "Ending" . ( is_null($child_treatment) ? " SELF" : " from CHILD: " . $child_treatment->name . " (" . $child_treatment->id . ")" ) , "result: " . $treatmentresult->value . ", completed: " . ($complete_treatment ? "YES" : "NO") . ", Child result: " . $child_treatment?->treatmentresult?->result?->value . ", Child state: " . $child_treatment?->state?->value);
    }
